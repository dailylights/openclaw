...
Crabbox inspect reports a primary SSH port and may report ordered fallback candidates. After Crabbox has authenticated the lease and marked it ready, OpenClaw selects the first candidate reachable from the Gateway host. If your network policy pins SSH ingress, allow at least one configured Crabbox candidate.
...
- **No SSH candidate is reachable** — Crabbox may authenticate readiness through a fallback port. OpenClaw honors the ordered candidates from inspect and chooses the first TCP-reachable one. Ensure the Gateway's outbound route and the worker's ingress policy allow at least one configured candidate.
