...
import {
  comparePluginSdkApiDeclarations,
  normalizePluginSdkApiSourcePath,
  printPluginSdkApiDeclaration,
} from "./api-baseline-declarations.js";
...
function createCompilerContext(repoRoot: string, entrypoints: readonly string[]) {
  ...
  const fileNames = entrypoints
    ...
    .toSorted((left, right) =>
      compareText(
        normalizePluginSdkApiSourcePath(repoRoot, path.resolve(left)),
        normalizePluginSdkApiSourcePath(repoRoot, path.resolve(right)),
      ),
    );
...
function buildSourceLink(repoRoot: string, filePath: string): PluginSdkApiSourceLink {
  return {
    path: normalizePluginSdkApiSourcePath(repoRoot, filePath),
  };
}
...
function resolveSymbolAndDeclaration(
  ...
  const declarations = (
    ...
    []
  ).toSorted((left, right) => comparePluginSdkApiDeclarations(repoRoot, left, right));
